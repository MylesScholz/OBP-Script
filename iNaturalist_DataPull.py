...
OBSERVATIONATTRIBUES = ["id","observed_on","time_observed_at","user_id","user_login","created_at",
                        "url","place_guess","latitude","longitude","positional_accuracy","place_county_name",
                        "place_state_name","place_country_name","scientific_name","taxon_family_name","field:sample id.","field:number of bees collected"]
...
class Observation:
    ...
    def __init__(self, d):
        ...
        self.field_number_of_bees_collected = get_value_from_ofvs(BEESCOLLECTEDNAME, d["ofvs"])
...
def get_observations(outside, year):
    ...
    observationsDict = pyinaturalist.v1.observations.get_observations(d1=minPullDate, project_id=projectId, per_page=200)
    ...
    if(int(observationsDict["total_results"]) > 200):
        ...
        while(i*200 < int(observationsDict["total_results"])):
            ...
            observationsDict["results"] = observationsDict["results"] + pyinaturalist.v1.observations.get_observations(d1=minPullDate, project_id=projectId, per_page=200, page=i)["results"]
...
def parse_cmd_line():
    ...
    if(yearArg):
        ...
        if(year.isnumeric() and int(year) >= currentYear):
            ...
            print("ERROR: Invalid year argument, must be a four digit year less than the current year")
            exit(1)
